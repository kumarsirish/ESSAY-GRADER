...
create index submissions_usn_idx on submissions (usn);
...
If your `submissions` table was created before retakes were supported (i.e. `usn` was the primary key), run this once to switch to one row per attempt instead of one row per USN:

```sql
alter table submissions drop constraint submissions_pkey;
alter table submissions add column id bigint generated by default as identity primary key;
alter table submissions add column created_at timestamptz default now();
create index if not exists submissions_usn_idx on submissions (usn);
```

### Schema notes
Each table stores its payload as a single `jsonb` column rather than one column per field:
- `submissions`: one row per **attempt** — `usn` is a plain indexed column, not a primary key, since a student can retake the test and accumulate multiple rows. `data` holds the full submission record (name, topic, essay, scores, etc.) exactly as the app builds it in Python.
...
- Students can retake the test as many times as needed; each attempt is stored as its own entry, so prior attempts are never overwritten.
